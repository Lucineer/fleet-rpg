...
- **Vessel**: Fleet Rpg
- **Role**: Fleet RPG
- **URL**: https://fleet-rpg.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/fleet-rpg
- **Branch**: master
...
### Deploy
```bash
cd /tmp/fleet-rpg && wrangler deploy
```

### Health Check
```bash
curl -s https://fleet-rpg.casey-digennaro.workers.dev/health
curl -s https://fleet-rpg.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=fleet-rpg
...
- **vessel.json capabilities**: canvas, rpg, stats
...
1. Keep the vessel small. ~363 lines is the current size.
